...
import static DATCOL.get_Courseshref.courseClass;
import static DATCOL.get_Courseshref.courseRoot;
import static DATCOL.get_Courseshref.courseSubClass;
import static DATCOL.get_Courseshref.course_desc;
import static DATCOL.get_Courseshref.course_duration;
import static DATCOL.get_Courseshref.course_entryrequirements;
import static DATCOL.get_Courseshref.course_fees;
import static DATCOL.get_Courseshref.course_feesplan;
import static DATCOL.get_Courseshref.course_field;
import static DATCOL.get_Courseshref.course_language;
import static DATCOL.get_Courseshref.course_location;
import static DATCOL.get_Courseshref.course_qualification;
import static DATCOL.get_Courseshref.course_startdate;
import static DATCOL.get_Courseshref.course_time;
import static DATCOL.get_Courseshref.course_title;
import static DATCOL.get_Courseshref.course_url;
import static DATCOL.get_Courseshref.course_wprovider;
import static DATCOL.get_Courseshref.search_key;
import static DATCOL.get_Courseshref.search_language;
import static DATCOL.get_Courseshref.uni_address;
import static DATCOL.get_Courseshref.uni_postal;
 ...
 * @author AhmedShalaby
 */
public class API_getCourses extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                for (int pageNum = 1; pageNum < 30; pageNum++) {
                    ...
                    search_key = request.getParameter("searchKey");
                    courseRoot = request.getParameter("rootClass");
                    courseClass = request.getParameter("mainClass");
                    courseSubClass = request.getParameter("subClass");

                    //get course url
                    String URL = "https://digital.ucas.com/search/results?SearchText=" + search_key.replace(" ", "%20") + "&AutoSuggestType=&SearchType=searchbarbutton&PreviouslyAppliedFilters=D_0_Postgraduate__&filters=Destination_Postgraduate&ProviderText=&SubjectText=&DistanceFromPostcode=1mi&RegionDistancePostcode=&SortOrder=0&CurrentView=List&pageNumber=" + pageNum;
...
//                        System.out.print(i + ": " + course_name + "- Page# " + pageNum + "- Courses# " + urlList.length + " \n" + course_url + "\n");
            ...
            out.print(json.printJson("status", "Data Collection Proccess: completed"));

        } catch (InstantiationException ex) {
            Logger.getLogger(API_getCourses.class.getName()).log(Level.SEVERE, null, ex);
